...
excel_directory = r"C:\Users\PMD - FEMI\Desktop\DELTA PROVINCE 20\QUARTER 1"
...
def clean_first_three_sheets(df):
    ...
    custom_header = ['S/N', 'PARISH NAME', 'BENCHMARK (AUG. 2023.)', 'SEP-23', 'OCT-23', 'NOV-23', 'ACTUAL AVG.SEP-23 - NOV-23']
    ...
    df_sorted = df_cleaned.sort_values(by=['ACTUAL AVG.SEP-23 - NOV-23'], ascending=False, ignore_index=True)
    ...
    return df_result
# Function to add columns to AVG. ATTENDANCE sheet
def add_columns_to_avg_attendance(df): 
    # Iterate through each row in the DataFrame
    for index, row in df.iterrows():
        # Check if the value in 'BENCHMARK (AUG. 2023.)' is zero
        if row['BENCHMARK (AUG. 2023.)'] == 0:
            ...
            if not pd.isnull(non_zero_index):
                ...
                df.at[index, 'BENCHMARK (AUG. 2023.)'] = row[non_zero_index]
   
    # Calculate 'VISION 2032 Expected Growth @ Q1' based on a formula
    df['VISION 2032 EXPECTED AVERAGE GROWTH @ Q1'] = df['BENCHMARK (AUG. 2023.)'] + (df['BENCHMARK (AUG. 2023.)'] * 0.10334)
   
    # Check if the 'ACTUAL AVG.SEP-23 - NOV-23' column exists in the DataFrame
    if 'ACTUAL AVG.SEP-23 - NOV-23' in df.columns:
        # Calculate 'Achieved GROWTH @ Q1' and add a new column
        df['ACHIEVEMENT OF TARGET GROWTH(IN %) @ Q1'] = (df['ACTUAL AVG.SEP-23 - NOV-23'] / df['VISION 2032 EXPECTED AVERAGE GROWTH @ Q1']) * 100
        # Format the '% Growth' column as a percentage
        df['ACHIEVEMENT OF TARGET GROWTH(IN %) @ Q1'] = df['ACHIEVEMENT OF TARGET GROWTH(IN %) @ Q1'].apply(lambda x: f'{x:.2f}%')
    else:
        print("Column 'ACTUAL AVG.SEP-23 - NOV-23' not found in DataFrame.")

    return df

# Function to add the '% GROWTH' column: for house fellowship and convert
def add_growth_HFC_column(df):
    ...
    df['% GROWTH'] = ((df['ACTUAL AVG.SEP-23 - NOV-23'] - df['BENCHMARK (AUG. 2023.)']) / df['BENCHMARK (AUG. 2023.)']) * 100
...
for file in excel_files:
    ...
    for sheet_name in list(all_sheets_dict.keys())[:3]:
        ...
        if sheet_name == 'AVG. ATTENDANCE':
            all_sheets_dict[sheet_name] = add_columns_to_avg_attendance(all_sheets_dict[sheet_name])
        elif sheet_name in ['CONVERTS', 'HOUSE FELLOWSHIP']:
            all_sheets_dict[sheet_name] = add_growth_HFC_column(all_sheets_dict[sheet_name])
