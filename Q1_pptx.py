        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                if sheet_name in ["HOUSE FELLOWSHIP", "CONVERTS"]:
                    #data_extraction_method = [list(row)[1:] for row in copiedSheet.UsedRange.Value]# This include the last column('Ranking')
                    data_extraction_method = [list(row)[1:-1] for row in copiedSheet.UsedRange.Value]# Exclude the first column ('S/N') & last column ('RANKING') from data extraction
