    ...
    try:
        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                if slide_index == 8:  # For MRR CHART
                    left = Inches(5.2)
                    top = Inches(1.3)
                    width = Inches(8.2)
                    height = Inches(6.5)
    ...
    try:
        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                # Get data from the entire sheet
    ...
    try:
        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                # Define the area to cover on the slide based on slide_index
                if slide_index == 11:  # CSR Distribution
                    left = Inches(1.5)
                    top = Inches(1)
                    width = Inches(11.5)
                    height = Inches(5.5)
                elif slide_index == 7:  # MRR
                    left = Inches(1.5)
                    top = Inches(0.7)
                    width = Inches(11.8)
                    height = Inches(6.2)
                elif slide_index == 10:  # CSR Table 1
                    left = Inches(1.6)
                    top = Inches(0.3)
                    width = Inches(11.5)
                    height = Inches(7)
                elif slide_index == 21:  # Church Analysis
                    left = Inches(1.5)
                    top = Inches(0.4)
                    width = Inches(11.6)
                    height = Inches(6.5)
                elif slide_index == 18:  # House Fellowship
                    left = Inches(1.5)
                    top = Inches(0.5)
                    width = Inches(11.5)
                    height = Inches(6.7)
                elif slide_index == 15:  # Converts
                    left = Inches(1.4)
                    top = Inches(0.5)
                    width = Inches(11.6)
                    height = Inches(6.7)
    ...
    try:
        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                if sheet_name == 'CSR':
                    # Find the column index where "Total" is present in row 1 of column D
                    total_column = None
    ...
    try:
        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                if sheet_name == 'CHURCH ANALYSIS1':
                    # Find the column index where "CHURCH ANALYSI" is present in row 1 of column B
                    noparishes_column = None
                    for col in range(1, copiedSheet.UsedRange.Columns.Count + 1):
                        if copiedSheet.Cells(1, col).Value == 'TOTAL NO. OF PARISHES':
                            noparishes_column = col
                            break

                    if noparishes_column is not None:
                        # Calculate CA value and directly use it on the slide
                        lt100percent_value = round((copiedSheet.Cells(4, noparishes_column).Value +                                                    
                                                    copiedSheet.Cells(5, noparishes_column).Value + 
                                                    copiedSheet.Cells(6, noparishes_column).Value + 
                                                    copiedSheet.Cells(7, noparishes_column).Value) / 
                                                    copiedSheet.Cells(8, noparishes_column).Value * 100, )
                        
                        totalLT100_value = round((copiedSheet.Cells(4, noparishes_column).Value + 
                                                  copiedSheet.Cells(5, noparishes_column).Value + 
                                                  copiedSheet.Cells(6, noparishes_column).Value + 
                                                  copiedSheet.Cells(7, noparishes_column).Value) )
                        #cell_value = [copiedSheet.Cells(3, col).Value for col in range(nov_2023_column, nov_2023_column + 1)]
                        column = noparishes_column
                        totalnoparishes_value = round(copiedSheet.Cells(8, noparishes_column).Value)

                        # Create a new slide in the PowerPoint presentation
                        slide = presentation.slides[slide_index]

                        # Add data to the slide with individual positions for each textbox:left,top,width,height
                        # For percentage of parishes less than 100 
                        textbox1 = slide.shapes.add_textbox(Inches(5.9), Inches(5.5), Inches(1), Inches(0.5))
                        textbox1.text_frame.text = f"{lt100percent_value}%"

                        # For total number of parishes within a province
                        textbox2 = slide.shapes.add_textbox(Inches(1), Inches(5.1), Inches(1), Inches(0.5))
                        textbox2.text_frame.text = f"{totalnoparishes_value}"

                        # For total number of parishes less than 100 within a province
                        textbox3 = slide.shapes.add_textbox(Inches(5.2), Inches(5), Inches(1), Inches(0.5))
                        textbox3.text_frame.text = f"{totalLT100_value}"

                        # Add more text boxes as needed

                    else:
                        raise ValueError("Column 'Total No. of Parishes' not found in sheet 'CA'")
        ...
        for sheet in excelWorkbook.Sheets:
            if sheet.Name == sheet_name:
                ...
                if sheet_name == 'CONVERTS':
                    ...
                    # Add data to the slide with individual positions for each textbox:left,top,width,height
                    textbox1 = slide.shapes.add_textbox(Inches(2), Inches(4.7), Inches(1), Inches(0.5))
                    textbox1.text_frame.text = f"{round(result_value, 2)}%"  # Round to 2 decimal places 
    ...
    try:
        ...
        for file_name in os.listdir(folder_path):
            if file_name.endswith('.xlsx'):
                ...
                copy_chart_to_existing_slide(excel_path, 'MRR', presentation, 8)
                copy_chart_to_existing_slide(excel_path, 'CSR DISTRIBUTION', presentation, 12)
                copy_chart_to_existing_slide(excel_path, 'CHURCH ANALYSIS1', presentation, 22)

                # Instructions to copy table to slide
                copy_avg_attendance_to_slide(excel_path, 'AVG. ATTENDANCE', presentation, 4)
                #copy_avg_attendance_to_slide(excel_path, 'AVG. ATTENDANCE', presentation, 5, copy_count=16)
                copy_table_to_slide(excel_path, 'MRR', presentation, 7, font_size=Pt(15))
                copy_table_to_slide(excel_path, 'CSR', presentation, 10, font_size=Pt(15))
                copy_table_to_slide(excel_path, 'CSR DISTRIBUTION', presentation, 11, font_size=Pt(15))
                copy_table_to_slide(excel_path, 'CONVERTS', presentation, 15, font_size=Pt(11))
                copy_table_to_slide(excel_path, 'HOUSE FELLOWSHIP', presentation, 18, font_size=Pt(11))
                copy_table_to_slide(excel_path, 'CHURCH ANALYSIS1', presentation, 21, font_size=Pt(15))

                # Instructions to copy cells to slide
                copy_cells_to_slide(excel_path, 'AVG. ATTENDANCE', presentation, 5)
                copy_cells_to_slide(excel_path, 'HOUSE FELLOWSHIP', presentation, 19)
                copy_cells_to_slide(excel_path, 'CONVERTS', presentation, 16)
                copy_mrrcells_to_slide(excel_path, 'MRR', presentation, 8)
                copy_csrcells_to_slide(excel_path, 'CSR', presentation, 13)
                #copy_cacells_to_slide(excel_path, 'CHURCH ANALYSIS1', presentation, 23)
                recommendation_CA_slide(excel_path, 'CHURCH ANALYSIS1', presentation, 27)
                recommendation_MRR_slide(excel_path, 'MRR', presentation, 25)
                recommendation_CSR_slide(excel_path, 'CSR', presentation, 26)
                recommendation_HF_slide(excel_path, 'HOUSE FELLOWSHIP', presentation, 27)
                recommendation_CONVERTS_slide(excel_path, 'CONVERTS', presentation, 26)          
                recommendation_ATTENDANCE_slide(excel_path, 'AVG. ATTENDANCE', presentation, 25)
...
def main():
    if len(sys.argv) == 3:
        ...
        #pptx_path = r"C:\Users\PMD - FEMI\Desktop\PPTX_TEMPLATES\Q1template.pptx" #for provinces
        pptx_path = r"C:\Users\PMD - FEMI\Desktop\PPTX_TEMPLATES\Q1template4regions.pptx" #for regions
