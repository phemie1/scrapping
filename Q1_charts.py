...
# Function to add charts to an Excel file
def add_charts_to_excel(file_path):
    ...
    first_timers_data = Reference(mrr_sheet, min_col=2, min_row=3, max_col=4, max_row=3)
    attendance_data = Reference(mrr_sheet, min_col=2, min_row=4, max_col=4, max_row=4)

    # Create a bar chart for MRR
    mrr_chart = BarChart()
    mrr_chart.type = "col"
    mrr_chart.style = 10
    mrr_chart.title = "Q1 MEMBERS RETENTION DISTRIBUTION"
    ...
    # Add the chart to the MRR sheet
    mrr_sheet.add_chart(mrr_chart, "A12")
    ...
    csr_chart.style = 10
    csr_chart.title = "Q1 CSR DISTRIBUTION"
    csr_chart.x_axis.title = "CSR CATEGORIES"
    csr_chart.y_axis.title = "No. OF CSR PROJECTS"
    csr_chart.add_data(Reference(csr_sheet, min_col=5, min_row=2, max_row=csr_sheet.max_row - 1, max_col=5))
    ...
    # Add the chart to the CSR sheet
    csr_sheet.add_chart(csr_chart, "A12")
    ...
    # Add the chart to the Church Analysis sheet
    CA_sheet.add_chart(CA_chart, "A12")
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.xlsx'):
        ...
        # Process the Excel file and add charts
        add_charts_to_excel(file_path)
        # Print a statement once the charts are drawn
        print(f'Charts added to {file_name}')
